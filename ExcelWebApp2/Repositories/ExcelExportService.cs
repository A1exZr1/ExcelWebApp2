...
namespace ExcelWebApp2.Repositories
{
    public class ExcelExportService
    {
        public MemoryStream Export(List<ProcessedResultModel> data)
        {
            ...
            worksheet.Cell(1, 4).Value = "Выручка";
            worksheet.Cell(1, 5).Value = "Расходы на рекламу";
            worksheet.Cell(1, 6).Value = "Себестоимость";
            worksheet.Cell(1, 7).Value = "Чистая прибыль";
            worksheet.Cell(1, 8).Value = "% от выручки";
            ...
            for (int i = 0; i < data.Count; i++)
            {
                ...
                worksheet.Cell(row, 5).Value = item.AdvertisingCost;
                worksheet.Cell(row, 6).Value = item.PrimeCost;
                worksheet.Cell(row, 7).Value = item.NetProfit;
                worksheet.Cell(row, 8).Value = item.ProfitPercent;
            }
